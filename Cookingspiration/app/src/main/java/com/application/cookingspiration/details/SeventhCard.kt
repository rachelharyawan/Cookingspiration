...
class SeventhCard : AppCompatActivity() {
        ...
        call.enqueue(object: Callback<GenerateMeals> {
            override fun onResponse(call: Call<GenerateMeals>,
                ...
                if (response.body()!= null) {
                    response.body()!!.week.sunday.meals[0].title.also { namebreakfastTextView.text = it }
                    response.body()!!.week.sunday.meals[1].title.also { namelunchTextView.text = it }
                    response.body()!!.week.sunday.meals[2].title.also { namedinnerTextView.text = it }
                    response.body()!!.week.sunday.meals[0].sourceUrl.also { sourcebreakfastTextView.text = it }
                    response.body()!!.week.sunday.meals[1].sourceUrl.also { sourcelunchTextView.text = it }
                    response.body()!!.week.sunday.meals[2].sourceUrl.also { sourcedinnerTextView.text = it }
                    response.body()!!.week.sunday.nutrients.calories.also { caloriesTextView.text = it.toString() + "cal"}
                    response.body()!!.week.sunday.nutrients.carbohydrates.also { carbohydratesTextView.text = it.toString() + "g" }
                    response.body()!!.week.sunday.nutrients.fat.also { fatTextView.text = it.toString() + "g" }
                    response.body()!!.week.sunday.nutrients.protein.also { proteinTextView.text = it.toString() + "g" }
                }
            }
    ...
    override fun onOptionsItemSelected(item : MenuItem) : Boolean {
        when (item.itemId) {
            R.id.sharebutton -> {
                val title = "Menu for " + seventhdate.text.toString()
                val description1 = "Menu for " + seventhdate.text.toString() +
